function Build-LumenApi {
    <#
        .SYNOPSIS
            Update a PHP/Lumen API application.
        .DESCRIPTION
            Wraps Invoke-WebAppUpdate with Lumen-specific build steps:
            - Production: composer install --no-dev -o + artisan config:clear
            - Development (develop branch): composer install (with dev dependencies)
        .PARAMETER Directory
            Path to the Lumen application directory.
        .PARAMETER Branch
    ...
    Process {
        ...
        $arguments['ScriptBlock'] = [ordered]@{
            Building   = {
                Write-Verbose '> Building Lumen for Production'
        ...
        if ($Branch -eq 'develop') {
            $arguments['ScriptBlock']['Building'] = {
                Write-Verbose '> Building Lumen for Development'
